...
7. 모든 skills 파일 생성 후 각 파일의 길이/sha256을 계산해 무결성을 검증한다.
   - `start_auto_flow(step=VERIFY_SKILL, flowId=..., skillId=..., writtenLength=..., writtenSha256=...)`
   - selectedSkills 전체가 검증 완료될 때까지 반복한다.
8. 사용자에게 "이대로 진행" 또는 "사용자 맞춤 보정" 중 하나를 확인한다.
...
9. 사용자 결정이 확인되면
   `start_auto_flow(step=DECIDE, flowId=..., userDecisionConfirmed=true, decision=ACCEPT|CUSTOMIZE, customizationNotes=...)`
   를 호출한다.
10. DECIDE 완료 후에만 FINALIZE를 호출한다.
   - ACCEPT: `start_auto_flow(step=FINALIZE, flowId=..., customizationApplied=false)`
   - CUSTOMIZE: 기존 skills 파일을 부분 보정한 뒤
     `start_auto_flow(step=FINALIZE, flowId=..., customizationApplied=true)`
11. FINALIZE 응답의 `actions.writeFiles`와 `actions.deleteFiles`를 즉시 반영한다.
12. 최종 상태(생성/수정/삭제 파일 목록)를 사용자에게 보고하고 종료한다.
...
- `VERIFY_SKILL` 검증 없이 `DECIDE` 또는 `FINALIZE`로 넘어갈 수 없다.
- `DECIDE` 없이 `FINALIZE` 호출은 서버가 거절한다.
- `CUSTOMIZE` 결정이면 보정 완료(`customizationApplied=true`) 없이 FINALIZE 할 수 없다.
