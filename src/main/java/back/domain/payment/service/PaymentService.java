...
public class PaymentService {
        ...
        subscriptionRepository.findByMemberId(memberId)
                .ifPresent(subscription -> {
                    if(subscription.isActive()){
                        throw new IllegalArgumentException("이미 활성화된 구독이 있습니다.");
                    }
                });
    ...
    //    결제 승인하기
    @Transactional
    public void confirm(Long memberId, PaymentConfirmRequest request) {
        // 1. DB에서 결제 대기 데이터 조회
        ...
        private void confirmToToss(PaymentConfirmRequest request) {
            // [주의] 실제 서비스 시에는 Secret Key를 설정 파일(yml)로 관리하세요!
